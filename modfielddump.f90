...
!> Initializing fielddump. Read out the namelist, initializing the variables
  subroutine initfielddump
    ...
    if (lnetcdf) then
      ...
      call ncinfo(ncname(46,:),'ssacti', 'Sea salt mass activation tendency',               '1e5 kg kg-1 s-1','tttt')
      call ncinfo(ncname(47,:),'ssscvc', 'Sea salt mass in-cloud scavenging tendency',      '1e7 kg kg-1 s-1','tttt')
      call ncinfo(ncname(48,:),'ssevpc', 'Sea salt mass cloud evaporation tendency',        '1e5 kg kg-1 s-1','tttt')
      call ncinfo(ncname(49,:),'ssscvr', 'Sea salt mass below-cloud scavenging tendency',   '1e5 kg kg-1 s-1','tttt')
      call ncinfo(ncname(50,:),'ssevpr', 'Sea salt mass rain evaporation tendency',         '1e5 kg kg-1 s-1','tttt')
      call ncinfo(ncname(51,:),'ssauto', 'Sea salt mass autoconversion tendency',           '1e5 kg kg-1 s-1','tttt')
      call ncinfo(ncname(52,:),'ssaccr', 'Sea salt mass accretion tendency',                '1e5 kg kg-1 s-1','tttt')
      call ncinfo(ncname(53,:),'sssedr', 'Sea salt mass rain sedimentation tendency',       '1e5 kg kg-1 s-1','tttt')
...
!> Do fielddump. Collect data to truncated (2 byte) integers, and write them to file
  subroutine fielddump
    ...
    use modmicrodata, only : aer_acti, aer_scvc, aer_evpc, aer_scvr, aer_evpr, aer_auto, aer_accr, aer_sedr
    ...
    if(imicro/=imicro_none) then
      ...
      do j=2-jh,j1+jh
      do k=1,k1
        field(i,j,k) = NINT(1.0E7*svm(i,j,k,iqr),2)
      enddo
      enddo
      enddo
    else
    ...
    if(imicro/=imicro_none) then
      ...
      do j=2-jh,j1+jh
      do k=1,k1
        field(i,j,k) = NINT(1e-2*svm(i,j,k,inr+iaer_offset),4)
      enddo
      enddo
      enddo
    else
    ...
    if(imicro/=imicro_none) then
      ...
      do j=2-jh,j1+jh
      do k=1,k1
        field(i,j,k) = NINT(1.0E5*aer_acti(i,j,k,isscld),4)
      enddo
      enddo
      enddo
    else
    ...
    if (lnetcdf) vars(:,:,:,46) = field(2:i1,2:j1,klow:khigh)
    if (lbinary) then
      if (ldiracc) then
        open (ifoutput,file='wbqr.'//cmyidx//'.'//cmyidy//'.'//cexpnr,access='direct', form='unformatted', recl=reclength)
        write (ifoutput, rec=writecounter) field(2:i1,2:j1,klow:khigh)
    ...
    if(imicro/=imicro_none) then
      ...
      do j=2-jh,j1+jh
      do k=1,k1
        field(i,j,k) = NINT(1.0E7*aer_scvc(i,j,k,isscld),4)
      enddo
      enddo
      enddo
    else
    ...
    if (lnetcdf) vars(:,:,:,47) = field(2:i1,2:j1,klow:khigh)
    if (lbinary) then
      if (ldiracc) then
        open (ifoutput,file='wbqr.'//cmyidx//'.'//cmyidy//'.'//cexpnr,access='direct', form='unformatted', recl=reclength)
        write (ifoutput, rec=writecounter) field(2:i1,2:j1,klow:khigh)
    ...
    if(imicro/=imicro_none) then
      ...
      do j=2-jh,j1+jh
      do k=1,k1
        field(i,j,k) = NINT(1.0E5*aer_evpc(i,j,k,isscld),4)
      enddo
      enddo
      enddo
    else
    ...
    if (lnetcdf) vars(:,:,:,48) = field(2:i1,2:j1,klow:khigh)
    if (lbinary) then
      if (ldiracc) then
        open (ifoutput,file='wbqr.'//cmyidx//'.'//cmyidy//'.'//cexpnr,access='direct', form='unformatted', recl=reclength)
        write (ifoutput, rec=writecounter) field(2:i1,2:j1,klow:khigh)
    ...
    if(imicro/=imicro_none) then
      ...
      do j=2-jh,j1+jh
      do k=1,k1
        field(i,j,k) = NINT(1.0E5*aer_scvr(i,j,k,issrai),4)
      enddo
      enddo
      enddo
    else
    ...
    if (lnetcdf) vars(:,:,:,49) = field(2:i1,2:j1,klow:khigh)
    if (lbinary) then
      if (ldiracc) then
        open (ifoutput,file='wbqr.'//cmyidx//'.'//cmyidy//'.'//cexpnr,access='direct', form='unformatted', recl=reclength)
        write (ifoutput, rec=writecounter) field(2:i1,2:j1,klow:khigh)
    ...
    if(imicro/=imicro_none) then
      ...
      do j=2-jh,j1+jh
      do k=1,k1
        field(i,j,k) = NINT(1.0E5*aer_evpr(i,j,k,issrai),4)
      enddo
      enddo
      enddo
    else
    ...
    if (lnetcdf) vars(:,:,:,50) = field(2:i1,2:j1,klow:khigh)
    if (lbinary) then
      if (ldiracc) then
        open (ifoutput,file='wbqr.'//cmyidx//'.'//cmyidy//'.'//cexpnr,access='direct', form='unformatted', recl=reclength)
        write (ifoutput, rec=writecounter) field(2:i1,2:j1,klow:khigh)
    ...
    if(imicro/=imicro_none) then
      ...
      do j=2-jh,j1+jh
      do k=1,k1
        field(i,j,k) = NINT(1.0E5*aer_auto(i,j,k,issrai),4)
      enddo
      enddo
      enddo
    else
    ...
    if (lnetcdf) vars(:,:,:,51) = field(2:i1,2:j1,klow:khigh)
    if (lbinary) then
      if (ldiracc) then
        open (ifoutput,file='wbqr.'//cmyidx//'.'//cmyidy//'.'//cexpnr,access='direct', form='unformatted', recl=reclength)
        write (ifoutput, rec=writecounter) field(2:i1,2:j1,klow:khigh)
    ...
    if(imicro/=imicro_none) then
      ...
      do j=2-jh,j1+jh
      do k=1,k1
        field(i,j,k) = NINT(1.0E5*aer_accr(i,j,k,issrai),4)
      enddo
      enddo
      enddo
    else
    ...
    if (lnetcdf) vars(:,:,:,52) = field(2:i1,2:j1,klow:khigh)
    if (lbinary) then
      if (ldiracc) then
        open (ifoutput,file='wbqr.'//cmyidx//'.'//cmyidy//'.'//cexpnr,access='direct', form='unformatted', recl=reclength)
        write (ifoutput, rec=writecounter) field(2:i1,2:j1,klow:khigh)
    ...
    if(imicro/=imicro_none) then
      ...
      do j=2-jh,j1+jh
      do k=1,k1
        field(i,j,k) = NINT(1.0E5*aer_sedr(i,j,k,issrai),4)
      enddo
      enddo
      enddo
    else
    ...
    if (lnetcdf) vars(:,:,:,53) = field(2:i1,2:j1,klow:khigh)
    if (lbinary) then
      if (ldiracc) then
        open (ifoutput,file='wbqr.'//cmyidx//'.'//cmyidy//'.'//cexpnr,access='direct', form='unformatted', recl=reclength)
        write (ifoutput, rec=writecounter) field(2:i1,2:j1,klow:khigh)
